...
Servers (master one and VPN ones) use an authorization key to communicate with each other via API. It is placed in a text file “key.txt” in “_data” folders of servers. Without a same key on both servers no communication is possible. The key is an alphanumeric string of ANY length (recommended length is from 256 to 512) and is also used to perform administrative actions in "/admin" web panel.
...
Provisioning of configs (unique connection strings) on severs are on behalf of VPN servers; the master server only serves, stores and provides information about existing configs and clients. The reason of such logic lies in instability of VPN servers - they may be deleted/banned.

Expanding a new VPN server on a new linux machine is extra easy. A bash script is used to accomplish this. During install, just some certain parameters should be entered and the server is ready to accept new clients and create new configs.
